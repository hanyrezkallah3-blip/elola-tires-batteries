...
export function transformUseOrderStore(file) {
  ...
  visit(ast, {
    ...
    ImportDeclaration(path) {
      ...
      if (value.endsWith('/store/orderStore')) {
    ...
    CallExpression(path) {
      ...
      if (!ORDER_FIELDS.includes(property)) {

        return

      }

      node.callee = t.identifier('useOrderStore')
  ...
  if (!hasOrderImport) {
    ...
    let importPath = '../store/orderStore'

    if (file.endsWith('App.jsx')) {

      importPath = './store/orderStore'
